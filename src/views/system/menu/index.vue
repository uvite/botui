...
<script setup>
  ...
  const crud = reactive({
    ...
    beforeAdd: (form) => {
      ...
      alert(3)
      // form.meta.icon = currentRow.value["meta.icon"]
      // form.meta.type = currentRow.value["meta.type"]
    },
    beforeEdit: (form) => {
      console.log("-------------==========")
      form.meta.icon=form["meta.icon"]
      form.meta.type=form["meta.type"]
      console.log(form.meta)

      // form.meta.icon = currentRow.value["meta.icon"]
      // form.meta.type = currentRow.value["meta.type"]
    },
    add: { show: true, api: menu.save  },
    edit: { show: true, api: menu.update  },
    delete: {
      show: true,
      api: menu.deletes ,
      realApi: menu.realDeletes
    },
    recovery: { show: true, api: menu.recoverys },
  ...
  const columns = reactive([
    { title: 'ID', dataIndex: 'id', addDisplay: false, editDisplay: false, width: 50, hide: true },
    {
      ...
      dict: { url: 'system/menu/tree',method:"post",props:{customField:"name",title:"name",key:"id"}, params: { onlyMenu: true } }, addDefaultValue: 0,
    ...
    {
      title: '菜单类型', dataIndex: 'meta.type', formType: 'radio', addDefaultValue: 'M', width: 100,
      dict: {
    ...
    {  title: '图标', dataIndex: 'meta.icon', width: 80, formType: 'icon-picker', style: { width: '100%' } },
    {
