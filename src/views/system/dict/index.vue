    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef">
      ...
      <template #code="{ record }">
        <a-tooltip content="点击查看字典数据" v-if="! isRecovery">
          <a-link @click="openDictList(record)">{{ record.type }}</a-link>
        </a-tooltip>
...
<script setup>
  ...
  const crud = reactive({
    ...
    operationWidth: 240,
    add: { show: true, api: dictType.save  },
    edit: { show: true, api: dictType.update  },
    delete: {
      show: true,
      api: dictType.deletes,
      realApi: dictType.realDelete,
    },
    recovery: { show: true, api: dictType.recoverys},
  })
  ...
  const columns = reactive([
    ...
    {
      title: '字典标识', dataIndex: 'type', search: true, width: 260,
