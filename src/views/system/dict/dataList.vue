...
const crud = reactive({
  ...
  beforeAdd: (form) => {
    ...
    form.sysDictionaryID = currentRow.value?.id
  },
  add: { show: true, api: dict.saveDictData  },
  edit: { show: true, api: dict.updateDictData  },
  delete: {
    show: true,
    api: dict.deletesDictData,
    realApi: dict.realDeletesDictData
  },
  recovery: { show: true, api: dict.recoverysDictData  },
  beforeRequest: (params) => params.sysDictionaryID = currentRow.value.id
})
