...
<script setup>
  ...
  const picPath = ref('')
  const form = reactive({ username: 'admin', password: '123456', captcha: '',captchaId:"",openCaptcha:"" })
  ...
  // 获取验证码
  const loginVerify = () => {
    loginApi.getCaptch({}).then(async(ele) => {
      // rules.captcha.push({
  ...
  <div class="login-container">
    <div class="login-width mx-auto flex justify-between h-full items-center">
      <div class="w-6/12 mx-auto left-panel rounded-l pl-5 pr-5 hidden md:block bg-blue-50">
        <div class="logo"><img src="/logo.svg" width="45"><span>知之量化</span></div>
